...
export default React.memo(() => {
  ...
  useEffect(() => {
    ...
    const { msgid } = params;
    // const msgid = '800b3f14-780f-4dde-afc5-fe7938082351';
    const p = Promise.race([
      api.getbHReportController(msgid, true).then((res: any) => {
        ...
        const contitleList =  [
          ...
          { title: '日新加好友', dataIndex: 'isbuyNumDate', width: 80 },
  ...
  const contitleList1 = [
    { title: dataType, dataIndex: 'dataTypeName', width: 98 },
    { title: '导购数字化', dataIndex: 'digiRate', width: 80 },
    { title: '导购数字化排名', dataIndex: 'digiRateRank', width: 80 },
    { title: '在职人数', dataIndex: 'sellerNumQq', width: 70 },
    { title: '创建商品达标人数', dataIndex: 'itemNumQq', width: 80 },
    { title: '创建销售单达标人数', dataIndex: 'sellNumQq', width: 90 },
    { title: '在线销售达标人数', dataIndex: 'saleNumQq', width: 80 },
    { title: '日创建商品人数', dataIndex: 'itemNumDate', width: 80 },
    { title: '日创建销售单人数', dataIndex: 'sellNumDate',  width: 80 },
    { title: '日在线销售人数', dataIndex: 'saleNumDate', width: 80 },
  ]
          ...
          <div className={styles.boxShadow}>
            <div className={styles.totalTitle}>{reportDate}</div>
              <div className={styles.tagWrap}>
                  {
                    totalTagList && (totalTagList.map((item, index) => {
                      var active = index === tipsVisible && true;
                       return (<GroupsTab active={active} data={item} key={index} handleClick={() => handleClick(index)}/>)
                    }))
                  }
              </div>
            </div>
            ...
            <div className={styles.wrap}>
              <div className={styles.content}>
                <div className={styles.content}> 
                  <GroupsTable sumList={titleList} dataList={typeDetailList} showType={tipsVisible} />                                  
                </div>
              </div>
            </div>
        </>
